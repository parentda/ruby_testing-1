...
describe NumberGame do
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      subject(:incomplete_game) { described_class.new(5, '9') }

      # remove the 'x' before running this test
      it 'is not game over' do
        expect(incomplete_game).to_not be_game_over
      end
    end
  end
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs an incorrect value once, then a valid input' do
      ...
      before do
        ...
        letter = 'd'
        valid_input = '8'
        allow(game_loop).to receive(:player_input).and_return(
          letter,
          valid_input
        )
      end
  ...
  describe '#final_message' do
    ...
    context 'when count is 2-3' do
      ...
      it 'outputs correct phrase' do
        congrats_phrase =
          "Congratulations! You picked the random number in 3 guesses!\n"
    ...
    context 'when count is 4 and over' do
      subject(:game_three) { described_class.new(7, '7', 7) }
