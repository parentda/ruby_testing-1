...
describe BinaryGame do
  ...
  describe '#verify_input' do
    ...
    context 'when given a valid input as argument' do
      it 'returns valid input' do
        ...
        max = game_input.instance_variable_get(:@maximum)
        input = 5
        return_value = game_input.verify_input(min, max, input)
        expect(return_value).to eq(input)
      end
    end
  ...
  describe '#display_turn_order' do
    ...
    let(:binary_search_display) { instance_double(BinarySearch) }

    before do
      # You'll need to create a few method stubs.
      allow(binary_search_display).to receive(:make_guess)
      allow(binary_search_display).to receive(:update_range)
      allow(turn_display).to receive(:display_guess).with(binary_search_display)
    end

    # Command Method -> Test the change in the observable state
    it 'increases guess_count by one' do
      expect {
        turn_display.display_turn_order(binary_search_display)
      }.to change { turn_display.instance_variable_get(:@guess_count) }.by(1)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess' do
      expect(binary_search_display).to receive(:make_guess)
      turn_display.display_turn_order(binary_search_display)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends update_range' do
      expect(binary_search_display).to receive(:update_range)
      turn_display.display_turn_order(binary_search_display)
    end
