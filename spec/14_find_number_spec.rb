...
describe FindNumber do
  ...
  describe '#initialize' do
    ...
    context 'when adding method names to subject for multiple tests' do
      ...
      subject(:game_solution) do
        described_class.new(0, 9, random_number).answer
      end
...
describe FindNumber do
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      subject(:game_over) { described_class.new(0, 9, answer, 5) }
    ...
    context 'when guess and random_number are not equal' do
      let(:answer) { double('random_number', { value: 5 }) }
      subject(:game_continue) { described_class.new(0, 9, answer, 6) }
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      subject(:end_guess_game) { described_class.new(5, 8, number_range, 7) }
      before { end_guess_game.update_range }

      it 'updates min to the same value as max' do
        expect(end_guess_game.min).to eq(8)
      end

      it 'does not update max' do
        expect(end_guess_game.max).to eq(8)
      end
    end
  end
end
