...
describe Dog do
  ...
  subject(:bentley) { described_class.new('bentley', 'Australian Shepherd') }

  # Write a test using the second shared_example to test that dog responds to
  # talk ('WOOF!').
  context 'when Dog shares method names with other classes' do
    include_examples 'shared method name'
  end

  # remove the 'x' before running this test
  it 'is not barking' do
    expect(bentley).to_not be_barking
  end

  # remove the 'x' before running this test
  it 'is sleeping' do
    expect(bentley).to be_sleeping
  end
end
